...
pub struct Application {
    ...
    blob_data_dir: std::path::PathBuf,
}
...
impl Application {
    pub async fn build(configuration: Settings) -> Result<Self, anyhow::Error> {
        ...
        Ok(Self {
            port,
            server,
            blob_data_dir: configuration.vault.data_dir.clone(),
        })
    }
    ...
    pub async fn run_until_stopped(self) -> Result<(), std::io::Error> {
        crate::vault::prepare_blob_dirs(&self.blob_data_dir).await?;
...
fn build_router(
    ...
    let admin_routes = Router::<AppState>::new()
        ...
        .route("/threads", get(threads_index))
        .route("/threads/{thread_id}", get(thread_page))
        .route("/threads/{thread_id}/tree", get(thread_tree))
        .route("/threads/{thread_id}/search", get(thread_search))
        .route("/threads/{thread_id}/share", post(create_share))
        .route("/vault", get(vault_admin))
        .route("/vault/search", get(global_search))
        .route("/vault/clients", post(create_api_key))
        .route("/vault/clients/{client_id}/revoke", post(revoke_api_key))
        .route("/vault/shares/{share_id}/revoke", post(revoke_share))
    ...
    Router::<AppState>::new()
        ...
        .route(
            "/s/{share_token}",
            get(get_shared_thread).post(password_share),
        )
