function _classesFromArray(index, array, split, github) {
...
function _widthClassFromArray(array) {
	const length = array.length;
	let output = "";
	output += "col-xs-";
	output += 12 / length;
...
function _textAlignFromArray(index, array) {
	const length = array.length;
	let output = "";
	if (length === 2) {
		if (index === 0) {
...
function _createList(array, sort = false) {
	...
	for (let i = 1; i < array.length; i++) output += `${delimiter}<span>${array[i]}</span>`;
	return output;
}

function _getIcon(icon = "", color = "", size = "") {
	// console.log(icon);
	if (icon) {
		...
		$(list)
			...
			.each((index, span) => {
				...
				if ($(span).offset().top !== previous.offset().top) {
					...
					if (previous.is(".delimiter")) {
						//If it was a delimiter, let's hide it
						const prevTop = previous.offset().top;
						...
						//If it was not a delimiter, then we hide current IF the text didn't wrap
						$(span).hide();
					}
				}
				previous = $(span);
...
ko.bindingHandlers.handlePageBreak = {
	...
	update: (_list, value) => {
		...
		$("main")
			.children()
			.each((_index, div) => {
				const divBottom = $(div).offset().top + $(div).height();
				// console.log(div);
...
function _convertToPDF(content) {
