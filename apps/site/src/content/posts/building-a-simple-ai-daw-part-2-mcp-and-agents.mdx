...
date: "2025-08-12"
subtitle: "A semi-technical deep-dive into building a robust AI audio agent. Learn how a rust-based streaming engine and the Model Context Protocol (MCP) agent"
tags: ["music", "ai", "ai-agents", "audio", "rust", "langchain", "claude", "ffmpeg", "mcp", "tutorial"]
coverImage: "../../assets/images/covers/building-a-simple-ai-daw-part-2-mcp-and-agents-cover.jpg"
...
![Udio Sessions UI](../../assets/images/posts/building-a-simple-ai-daw-part-2-mcp-and-agents-image-3.jpg)
...
Or if you’d prefer to go straight into the code: [https://github.com/jonaylor89/freqmoda](https://github.com/jonaylor89/freqmoda)
...
![Mozart Chat View](../../assets/images/posts/building-a-simple-ai-daw-part-2-mcp-and-agents-image-4.jpg)
...
![A picture of a robot in the studio](../../assets/images/posts/building-a-simple-ai-daw-part-2-mcp-and-agents-image-5.jpg)

## **4. Improvements from Part 1**

💡 *For the engineers and tech savvy people, there’s a more detailed Technical Appendix at the end of this post, complete with code walkthroughs and architectural insights. You can jump straight there, or continue reading for the high-level overview.*  One of the main issues with the previous blog post’s toy agent was that it was extremely unreliable. Sometimes it would hallucinate FFmpeg parameters that didn’t exist, get confused where to save temporary files, or accidentally get itself in infinite loops. The root cause of this is that the agent has too much it needs to do that isn't involved in the actual music editing process: file management, remembering documentation, error correcting, etc.. The solution I created to this is a separate server/tool that automates all this for the agent which I called **Streaming Engine**. The Streaming Engine is a server that wraps FFmpeg and exposes its functions via an HTTP API. That lets the AI call tools over the network with no local setup.

![freqmoda architecture diagram](../../assets/images/posts/building-a-simple-ai-daw-part-2-mcp-and-agents-image-6.jpg)

The next major improvement was switching from the legacy LangChain prompting / parsing to run tools and use the new, fancy Model Context Protocol (MCP). The previous agent was originally built pre-MCP becoming the standard so it relied on just a custom [ReAct](https://arxiv.org/abs/2210.03629) prompting. There’s a [lot](https://www.anthropic.com/news/model-context-protocol) [of](https://modelcontextprotocol.io/overview) [articles](https://www.forbes.com/sites/davidbirch/2025/04/26/why-you-need-to-know-about-the-model-context-protocol/) explaining how MCP works so I won’t go into any detail here other than to say that the Streaming Engine could be easily connected to Claude, giving it access to a tool definition that looks like:
...
![Claude Desktop with custom MCP](../../assets/images/posts/building-a-simple-ai-daw-part-2-mcp-and-agents-image-7.jpg)

## **5. Demo**
 <YouTube id="BGSBQDlbwb8" />
...
- [**Johannes Naylor**](https://bio.jonaylor.com) – Engineer @ Udio, CEO @ Tapped Ai
- [**Pal Chohan**](https://www.linkedin.com/in/pal-chohan) – Former Product Lead for TikTok’s Music R&D, startup founder (SendMusic - used by all the major labels) and Mentor @ music-tech incubator Abbey Road Red & music producer
